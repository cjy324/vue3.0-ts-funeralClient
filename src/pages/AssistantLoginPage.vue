<template>
  ...
  <section class="section section-assistant-login-form px-2">
...
export default defineComponent({
  ...
  setup(props) {
    ...
    function login(loginId:string, loginPw:string) {
      mainApi.assistant_authKey(loginId, loginPw)
        .then(axiosResponse => {
          ...
          const loginedAssistant = axiosResponse.data.body.assistant;  //assistant클래스 가져오기

          localStorage.setItem("authKey", authKey);
          localStorage.setItem("loginedAssistantId", loginedAssistant.id + "");
          localStorage.setItem("loginedAssistantName", loginedAssistant.name);
          localStorage.setItem("loginedAssistantProfileImgUrl", loginedAssistant.extra__thumbImg);

          props.globalShare.loginedAssistant = {
            authKey,
            id:loginedAssistant.id,
            name:loginedAssistant.name,
            profileImgUrl:loginedAssistant.extra__thumbImg
