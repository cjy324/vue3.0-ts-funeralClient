<template>
  ...
  <section class="section section-expert-login-form px-2">
...
export default defineComponent({
  ...
  setup(props) {
    ...
    function login(loginId:string, loginPw:string) {
      mainApi.expert_authKey(loginId, loginPw)
        .then(axiosResponse => {
          ...
          const loginedExpert = axiosResponse.data.body.expert;  //expert클래스 가져오기

          localStorage.setItem("authKey", authKey);
          localStorage.setItem("loginedExpertId", loginedExpert.id + "");
          localStorage.setItem("loginedExpertName", loginedExpert.name);
          localStorage.setItem("loginedExpertProfileImgUrl", loginedExpert.extra__thumbImg);

          props.globalShare.loginedExpert = {
            authKey,
            id:loginedExpert.id,
            name:loginedExpert.name,
            profileImgUrl:loginedExpert.extra__thumbImg
